...
bool App::startup()
{
	...
	// Set client defaults
	client->Data().SetElement("Size", vec3(.4f, 1.5f, 0));
...
void App::draw()
{
	...
	DrawScene();
	DrawSceneUI();
	DrawClientUI(client->Data(), "Position");

	if (client->OtherObjects().contains(3 - client->ID()))
		DrawClientUI(client->OtherData(3 - client->ID()), "LocalPosition", false);
...
void App::SpawnBall()
{
	...
	ball.networkData.SetElement("Velocity", RandomDirection(m_ballSpeed));
...
void App::CheckInput(float _dt)
{
	...
	float oldPosY = pos.y; // store so we can revert to this position

	int verticalInput = input->isKeyDown(aie::INPUT_KEY_UP) - input->isKeyDown(aie::INPUT_KEY_DOWN);
	float MOVESPEED = 5.f;
	pos.y += MOVESPEED * _dt * verticalInput;
	vel.y = MOVESPEED * verticalInput;

	// Clamp pos / vel to keep client on screen
	vec3 size = client->Data().GetElement<vec3>("Size");

	if (ToWindowPos(pos).y > getWindowHeight() - ToWindowSize(size).y * 0.5f)
	{
		pos.y = oldPosY;
		vel.y = 0;
	}
	else if (ToWindowPos(pos).y < ToWindowSize(size).y * 0.5f)
	{
		pos.y = oldPosY;
		vel.y = 0;
	}
...
void App::CheckScreenCollision()
{
	...
	{
		ballVel = RandomDirection(m_ballSpeed);
		ballPos = vec3(0);
...
void App::DrawScene()
{
	...
	// Us
	vec2 wPos = ToWindowPos(client->Data().GetElement<vec3>("Position"));
	vec2 wSize = ToWindowSize(client->Data().GetElement<vec3>("Size"));
	m_2dRenderer->drawSprite(m_paddleTex, wPos.x, wPos.y, wSize.x, wSize.y);

	// Other client
	if (client->OtherObjects().contains(3 - client->ID()))
	{
		wPos = ToWindowPos(client->OtherData(3 - client->ID()).GetElement<vec3>("LocalPosition"));
		wSize = ToWindowSize(client->OtherData(3 - client->ID()).GetElement<vec3>("Size"));
		m_2dRenderer->drawSprite(m_paddleTex, wPos.x, wPos.y, wSize.x, wSize.y);
	}
...
void App::DrawClientUI(NetworkData _data, const char* _posKey, bool _isClient)
{
	vec2 wPos = ToWindowPos(_data.GetElement<vec3>(_posKey));
	...
	if (!m_gameStart)
	{
		...
		if (!ready && _isClient) // write text
		{
			const char* text = "Press 'space' to ready up";
			float textOffset = m_2dRenderer->measureTextWidth(m_font2Thirds, text) * 0.5f;
			vec2 textPos = vec2(getWindowWidth() * 0.5f - textOffset, getWindowHeight() * 0.75f);

			m_2dRenderer->setRenderColour(0, 0, 0);
			m_2dRenderer->drawText(m_font2Thirds, text, textPos.x, textPos.y);
		}
	}
